...
class BIEvaluation:
    ...
    def add_binding_interface_and_sasa_scores_info(self):
        ...
        self.ms_dsa["asa_avg"] = np.nan # This column will be added to the ms_dsa dataframe
        self.ms_dsa["asa_min"] = np.nan # This column will be added to the ms_dsa dataframe
        self.ms_dsa["asa_max"] = np.nan # This column will be added to the ms_dsa dataframe
        self.ms_dsa["rel_asa_avg"] = np.nan # This column will be added to the ms_dsa dataframe
        self.ms_dsa["rel_asa_min"] = np.nan # This column will be added to the ms_dsa dataframe
        self.ms_dsa["rel_asa_max"] = np.nan # This column will be added to the ms_dsa dataframe
        self.ms_dsa["asa_max"] = np.nan # This column will be added to the ms_dsa dataframe
        for index, row in self.ms_dsa.iterrows():
            ...
            for uniprot_id in uniprot_ids:
                ...
                binding_interfaces = BindingInterfacesStore(structure_db="pdb", cutoff=8.0).load(uniprot_id)
                if binding_interfaces is None:
                    warnings.warn(f"Binding interfaces not found for uniprot_id {uniprot_id}")
                    continue
                ...
                is_inter_chain_contacts = len(inter_chain_lysines) > 0 or is_inter_chain_contacts
                is_intra_chain_contacts = len(intra_chain_lysines) > 0 or is_intra_chain_contacts
                ...
                # self.ms_dsa.at[index, "asa_avg"] = np.mean(asa)
                # self.ms_dsa.at[index, "asa_min"] = np.min(asa)
                # self.ms_dsa.at[index, "asa_max"] = np.max(asa)
                # self.ms_dsa.at[index, "rel_asa_avg"] = np.mean(rel_asa)
                # self.ms_dsa.at[index, "rel_asa_min"] = np.min(rel_asa)
                # self.ms_dsa.at[index, "rel_asa_max"] = np.max(rel_asa)
