...
from dsa.binding_interfaces.process_structure.interval_dict import IntervalDict
...
class Structure:
    ...
    def __init__(self, structure_id: str, database: str = "pdb"):
        if database == "pdb":
            ...
            self.structure_dir = STRUCTURE_DIR / "alphafold"
        else:
        ...
        self.uniprot_to_entity = self.map_uniprot_to_internal_ids(id_type='entity_id')
        self.entity_to_subchains = self.entity_to_subchains()
        self.bio_structure = self.create_biological_structure()
    ...
    def chains(self) -> list[str]:
        return [c.name for c in self.structure[0]]
    ...
    def create_biological_structure(self, assembly_index: int = 0):
        ...
        bio_structure = gemmi.make_assembly(assembly, self.structure[0], gemmi.HowToNameCopiedChain.AddNumber)
        return bio_structure

    

    def initialize_binding_interfaces(self, cutoff: float = 8.0, lysine_only: bool = False) -> BindingInterfaceExtractor:
        self.BI_extractor = BindingInterfaceExtractor(self.structure, cutoff=cutoff, lysine_only=lysine_only)
    ...
    def map_uniprot_to_internal_ids(self, id_type: str = 'entity_id') -> dict[str, list[str]]:
        ...
        if self.structure_path.suffix == '.cif' and self.database == 'pdb':
            block = gemmi.cif.read(self.structure_path.as_posix()).sole_block()
            ID_table = block.find('_struct_ref.', [id_type, 'db_name', 'pdbx_db_accession'])
            for row in ID_table:
                if row[1].upper() == 'UNP':
                    uniprot_to_entity[row[2]].append(row[0])
        elif self.structure_path.suffix == '.pdb.gz' and self.database == 'alphafold':
            #structure_id is uniprot_id
            uniprot_to_entity = {self.structure_id: [1]}
    ...
    def __getstate__(self):
        state = {"structure_id": self.structure_id, "database": self.database}
        return state

    def __setstate__(self, state):
        self.__init__(state["structure_id"], state["database"])
    ...
    @staticmethod
    def get_saved_structures(structure_db: str, from_structures: list[str] = None) -> list[str]:
        if not Structure.SavedStructures[structure_db]:
            if structure_db == 'pdb':
                ...
                structure_ids = uniprot_ids_with_saved_alphafold_structures(format="pdb")
            else:
                ValueError(f"Invalid database: {structure_db}")
                return []
            if from_structures is not None:
                structure_ids = list(set(structure_ids) & set(from_structures))
    ...
    def extract_binding_interfaces(cls, structure_id: str, 
                                        ...
                                        only_from_saved_file: bool = False,
                                        save_contacts: bool = False
                                        ) -> dict[str, dict[str, list[tuple[int, str]]]]|None:
        structure_id = cls.canonicalize_structure_id(structure_id, database)
        is_BI_saved = cls.is_saved_binding_interfaces(structure_id, database, cutoff, lysine_only)
        ...
        if is_BI_saved and (try_saved_file or only_from_saved_file):
            contact_residues = cls.load_binding_interfaces(structure_id, database, cutoff, lysine_only)
        if not is_BI_saved and (not only_from_saved_file):
            try:
                structure = cls(structure_id, database=database)
                structure.initialize_binding_interfaces(cutoff=cutoff, lysine_only=lysine_only)
                contact_residues = structure.chainwise_contacts()
                if save_contacts:
                    cls.save_binding_interfaces(contact_residues, structure_id, database, cutoff, lysine_only)
...
if __name__ == "__main__":
    ...
    if db == "pdb":
        structure_mapper = UniprotStructureMappingsStore.get_mapper(structure_db=db)
        structures_ids = structure_mapper.get_mapped_structures()
    elif db == "alphafold":
        structures_ids = ALL_UNIPROT_IDS
    else:
        raise ValueError(f"Invalid database: {db}")
    structures_to_process = Structure.select_unprocessed_structures(structures_ids, db, cutoff, lysine_only)
    ...
    while len(structures_to_process) > 0:
        ...
        is_BI_saved = Structure.is_saved_binding_interfaces(structure_id, db, cutoff, lysine_only)
        if is_BI_saved:
            file_path = Structure.BI_path(structure_id, db, cutoff, lysine_only)
            print(f"Contacts for {structure_id} already saved at {file_path}")
            continue
        contact_residues = Structure.extract_binding_interfaces(structure_id, database=db, cutoff=cutoff, lysine_only=lysine_only, save_contacts=save_contacts)
        if contact_residues is None:
            print(f"Failed to extract contacts for {structure_id}")
            continue
        count_processed += 1
        if count_processed % log_interval == 0:
            structures_to_process = Structure.select_unprocessed_structures(structures_to_process, db, cutoff, lysine_only)
